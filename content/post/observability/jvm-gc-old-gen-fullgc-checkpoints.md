...
- 특정 pod 하나만 유독 빨리 차는가
- 아니면 모든 pod가 비슷한 패턴으로 old gen을 채우는가

특정 pod만 문제면 애플리케이션 상태 오염이나 특정 요청 편향 가능성이 크고, 전체 공통이면 코드 경로 또는 트래픽 특성 문제일 가능성이 더 크다.
...
이 경우는 단순한 순간 부하보다 `장수 객체 누적`, `과도한 캐시`, `실질적 메모리 누수` 쪽을 의심하는 것이 맞다.
...
특히 dominator tree 기준으로 어떤 객체군이 old gen 대부분을 잡고 있는지 먼저 보는 것이 빠르다.
...
- local cache 사용 여부
- max size 설정 여부
- expire after write / access 설정 여부
- refresh 전략
- key cardinality가 과도하게 커지지 않는지

캐시가 warm-up 중이라면 일시적으로 커질 수 있지만, steady state 이후에도 계속 올라가면 설정 또는 설계 점검이 필요하다.
...
한 번에 큰 배열이나 문자열을 만드는 코드가 있다면 G1 입장에서 불리할 수 있다.
...
애플리케이션이 살아 있어도, GC가 지나치게 비효율적이면 실질적으로 장애와 비슷한 상황이 될 수 있다.

## 빠른 해석 기준

### 정상에 가까운 경우

- old gen이 올라가도 이후 다시 의미 있게 내려온다
- Full GC가 드물다
- pause time이 짧고 안정적이다
- 특정 배치/피크 시간대 이후 회복된다

### 위험 신호에 가까운 경우

- old gen이 빠르게 상한까지 찬다
- evacuation failure가 반복된다
- Full GC가 자주 뜬다
- Full GC 후에도 사용량이 잘 안 내려간다
- pause time이 수 초 단위로 길어진다
...
한 줄로 정리하면, 현재 자료는 `GC가 많이 도는 상태`가 아니라 `old gen이 차오르고 Full GC도 잘 못 구해주는 상태`로 보는 것이 더 적절하다.
